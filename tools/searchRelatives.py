#!/usr/bin/python3
# -*- coding: utf-8 -*-

####
# Copyright (C) 2018 Kim Gerdes
# kim AT gerdes. fr
# http://arborator.ilpga.fr/
#
# This program is free software; you can redistribute it and/or
# modify it under the terms of version 3 of the GNU Affero General Public License (the "License")
# as published by the Free Software Foundation; either version 3
# of the License, or (at your option) any later version.
#
# This script is distributed in the hope that it will be useful,
# but WITHOUT ANY WARRANTY; without even the implied warranty of
# MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE
# See the GNU General Public License (www.gnu.org) for more details.
#
# You can retrieve a copy of of version 3 of the GNU Affero General Public License
# from http://www.gnu.org/licenses/agpl-3.0.html 
####

import re, copy, os, html.entities
from segtok.segmenter import split_single # https://github.com/fnl/segtok
from lib import conll, transconll, docopt

try:
	import tqdm
	tqdm.monitor_interval = 0
except:
	pass

import spacy
nlp_en = spacy.load('en')


def usub(m): 
	return html.entities.html5[m.group(0)[1:]]

def textCorrection(t):
	t = t.replace("&amp;","&")
	entity = re.compile(r"&\w+;")
	t = entity.sub(usub, t)
	
	return t

def toSentences(infile, outfile):
	"""
	
	"""
	triple = re.compile(r"\s*\n\s*\n\s*\n\s*",re.M)
	double = re.compile(r"\s*\n\s*\n\s*",re.M)
	sentpunct = re.compile(r"([!?.;]+)\s*")
	text = open(infile).read()
	outstr = ""
	for sect in triple.split(text):
		if len(double.split(sect)) !=2:
			sqdf
		t = textCorrection(double.split(sect)[1])
		t = sentpunct.sub(r"\1 ",t)
		t = "\n".join(split_single(t))
		outstr += t.strip()+"\n"
	open(outfile,"w").write(outstr)
		

def matchGrammar(sg,tree):
	matchingRoots = sg.findall(tree)
	#if matchingRoots: print("match")
	return matchingRoots
	
#toSentences("levelB2sp.txt","sp.txt")
#toSentences("levelB2fr.txt","fr.txt")

#to, when, which, who, that, no
gto="""
7	_	_	_	_	_	0	_	_	_
8	_	_	_	_	_	7	relcl	_	_
9	to	_	_	_	_	8	.*	_	_
""".strip()
sgto = transconll.SearchGrammar(gto)

gwhen="""
7	_	_	_	_	_	0	_	_	_
8	_	_	_	_	_	7	relcl	_	_
9	when	_	_	_	_	8	.*	_	_
""".strip()
sgwhen = transconll.SearchGrammar(gwhen)

gwhich="""
7	_	_	_	_	_	0	_	_	_
8	_	_	_	_	_	7	relcl	_	_
9	which	_	_	_	_	8	.*	_	_
""".strip()
sgwhich = transconll.SearchGrammar(gwhich)
gwhichsubj="""
7	_	_	_	_	_	0	_	_	_
8	_	_	_	_	_	7	relcl	_	_
9	which	_	_	_	_	8	nsubj.*	_	_
""".strip()
sgwhichsubj = transconll.SearchGrammar(gwhichsubj)
gwhichobj="""
7	_	_	_	_	_	0	_	_	_
8	_	_	_	_	_	7	relcl	_	_
9	which	_	_	_	_	8	dobj	_	_
""".strip()
sgwhichobj = transconll.SearchGrammar(gwhichobj)

gwho="""
7	_	_	_	_	_	0	_	_	_
8	_	_	_	_	_	7	relcl	_	_
9	who	_	_	_	_	8	.*	_	_
""".strip()
sgwho = transconll.SearchGrammar(gwho)
gwhosubj="""
7	_	_	_	_	_	0	_	_	_
8	_	_	_	_	_	7	relcl	_	_
9	who	_	_	_	_	8	nsubj.*	_	_
""".strip()
sgwhosubj = transconll.SearchGrammar(gwhosubj)
gwhoobj="""
7	_	_	_	_	_	0	_	_	_
8	_	_	_	_	_	7	relcl	_	_
9	who	_	_	_	_	8	dobj	_	_
""".strip()
sgwhoobj = transconll.SearchGrammar(gwhoobj)


gthat="""
7	_	_	_	_	_	0	_	_	_
8	_	_	_	_	_	7	relcl	_	_
9	that	_	_	_	_	8	.*	_	_
""".strip()
sgthat = transconll.SearchGrammar(gthat)
gthatsubj="""
7	_	_	_	_	_	0	_	_	_
8	_	_	_	_	_	7	relcl	_	_
9	that	_	_	_	_	8	nsubj.*	_	_
""".strip()
sgthatsubj = transconll.SearchGrammar(gthatsubj)
gthatobj="""
7	_	_	_	_	_	0	_	_	_
8	_	_	_	_	_	7	relcl	_	_
9	that	_	_	_	_	8	dobj	_	_
""".strip()
sgthatobj = transconll.SearchGrammar(gthatobj)

gno="""
7	_	_	_	_	_	0	_	_	_
8	_	_	_	_	_	7	relcl	_	_
9	_	_	_	_	_	8	.*	_	_
""".strip()
sgno = transconll.SearchGrammar(gno)





def getConll(doc, code=None):
	"""
	conll string from spacy parse	
	"""
	conll = "# "+doc.text+'\n'
	if code: conll+= "# "+code+'\n'
	for i, word in enumerate(doc):
		if word.head == word:
			head_idx = 0
		else:
			head_idx = word.head.i - doc[0].i + 1
		conll+="%d\t%s\t%s\t%s\t%s\t%s\t%s\t_\t_"%(
		i+1, # There's a word.i attr that's position in *doc*
		word,
		word.lemma_,
		word.tag_, # Fine-grained tag
		'_\t_',#word.ent_type_,
		str(head_idx),
		word.dep_ # Relation
		)+"\n"
	return conll

def classifyTree(tree):
	matches = {}
	tree.addspan()
	for mat in matchGrammar(sgno,tree):
		if mat in matchGrammar(sgto,tree): continue # skipping weird tough constructions that spacy analyzes as relatives
		ty=None
		relhead = [k for k,r in tree[mat]['kids'].items() if r=='relcl'][0] # index of the head word of the relative clause
		firstrelword=tree[min(tree[relhead]['span'])] # node of the first word of the relative clause
		if firstrelword['tag']=='IN': firstrelword=tree[min(tree[relhead]['span'])+1]
		lastrelword=tree[max(tree[relhead]['span'])] # node of the last word of the relative clause
		#print("firstrelword",relhead,firstrelword)
		if firstrelword.get('lemma',None)=='that':
			#print('***',firstrelword.get('gov',{}),'dobj' in firstrelword.get('gov',{}).values())
			ty = "that-"+firstrelword.get('govrel','strange!')
		elif firstrelword.get('lemma',None)=='who':
			ty = "who-"+firstrelword.get('govrel','strange!')
		elif firstrelword.get('lemma',None)=='which':
			ty = "which-"+firstrelword.get('govrel','strange!')
		elif firstrelword.get('lemma',None)=='where': ty = "where"
		elif firstrelword.get('lemma',None)=='why': ty = "why"
		if not ty:
			if mat in matchGrammar(sgthatsubj,tree): ty = "that-nsubj??"
			elif mat in matchGrammar(sgthatobj,tree): ty = "that-dobj??"
			elif mat in matchGrammar(sgwhosubj,tree): ty = "who-nsubj??"
			elif mat in matchGrammar(sgwhoobj,tree): ty = "who-dobj??"
			elif mat in matchGrammar(sgwhichsubj,tree): ty = "which-nsubj??"
			elif mat in matchGrammar(sgwhichobj,tree): ty = "which-dobj??"
			elif mat in matchGrammar(sgwhen,tree): ty = "when"
			else: 
				
				if lastrelword['tag'] in ['IN','RP']:
					ty = "pobj?"
				else:
					for i in tree[relhead]['span']:
						if tree[i]['tag'] in ['IN'] and len(tree[i]['span'])==1 and tree[i].get('govrel',None) not in ['mark','det'] and tree[i]['t'] not in ['that','to']:
							#print("$$$",tree.sentence,mat,tree,lastrelword)
							#print(tree.conllu())
							#print('lastrelword',lastrelword)
							#print('span',tree[relhead]['span'])
							#print(':::',mat,i,"\n\n\n")
							ty = "pobj??"
						
				if not ty:
					ty="no-dobj???"
		matches[mat]=ty
	return matches	
	  

def xmlToSentences(infile):
	outfile = infile.split(".")[0]+".sentences"
	infofile = infile.split(".")[0]+".info"
	conllfile = infile.split(".")[0]+".conllu"
	resfile = infile.split(".")[0]+".tsv"
	
	sentpunct = re.compile(r"([!?.;]+)\s*")
	double = re.compile(r"\s*\n\s*\n\s*",re.M)
	multispace = re.compile(r"  +")
	
	
	relearnat = re.compile(r'<learner id="(\d+)" nationality="(\w+)"',re.M) #<learner id="82909" nationality="fr"/>
	retop = re.compile(r'<topic id="(\d+)">(.*)</topic>',re.M) #<topic id="74">Doing a survey about discrimination</topic>
	retext = re.compile(r'<text>(.*)</text>',re.M+re.DOTALL) #<topic id="74">Doing a survey about discrimination</topic>

	
	text = open(infile).read()
	segs = text.split("<writing ")
	
	counter = 0
	with open(outfile,"w") as outf, open(infofile,"w") as infof, open(conllfile,"w") as conllf, open(resfile,"w") as resf:
		resf.write("\t".join(["counter", "wordNb", "relPro-relation", "headNoun", "sentence", "topicId", "topic", "nationality", "learner"])+'\n')
		for seg in segs:
			if '<text>' in seg:
				#print(seg)
				
				for m in relearnat.finditer(seg): learner,nationality = m.group(1), m.group(2)
				for m in retop.finditer(seg): topid, topic = m.group(1), m.group(2)
				for m in retext.finditer(seg): segtext= m.group(1).strip().replace('<br/>','\n').replace('&amp;quot;','"')
				segtext = double.sub('\n',segtext)
				segtext = multispace.sub(' ',segtext)
				
				for li in segtext.split('\n'):
					#print(4444,li)
					doc = nlp_en(li)
					for s in doc.sents:
						#print(s.text)
						outf.write(s.text+"\n")
						infof.write("\t".join([nationality,learner,topid, topic,s.text])+'\n')
						counter+=1
						conlltext = getConll(s, ", ".join([str(counter),nationality,learner,topid]))
						conllf.write(conlltext+'\n')
						tree = conll.conll2tree(conlltext)
						#print(tree)
						matches = classifyTree(tree)
						#print("___________",s.text)
						#print(conlltext)
						#print("matches:::::",matches)
						#print(tree)
						for mat in matches:
							#print("match:",mat, matches[mat])
							resf.write("\t".join([str(counter), str(mat-1), matches[mat], s[mat-1].text, s.text, topid, topic, nationality, learner, segtext.replace("\n","¶")])+'\n')
						
						if not counter%1000: print(counter)
					#if counter>427: qsdf
			#if counter>14: break
						
						
						
						
	print("wrote",counter,"sentences")
				


conlltree="""# Do you know someone who wants to bring a complaint about discrimination and what type of discrimination?
# 7, fr, 82909, 74
1	Do	do	VBP	_	_	3	aux	_	_
2	you	-PRON-	PRP	_	_	3	nsubj	_	_
3	know	know	VB	_	_	0	ROOT	_	_
4	someone	someone	NN	_	_	3	dobj	_	_
5	who	who	WP	_	_	6	nsubj	_	_
6	wants	want	VBZ	_	_	4	relcl	_	_
7	to	to	TO	_	_	8	aux	_	_
8	bring	bring	VB	_	_	6	xcomp	_	_
9	a	a	DT	_	_	10	det	_	_
10	complaint	complaint	NN	_	_	8	dobj	_	_
11	about	about	IN	_	_	10	prep	_	_
12	discrimination	discrimination	NN	_	_	11	pobj	_	_
13	and	and	CC	_	_	12	cc	_	_
14	what	what	WDT	_	_	15	det	_	_
15	type	type	NN	_	_	12	conj	_	_
16	of	of	IN	_	_	15	prep	_	_
17	discrimination	discrimination	NN	_	_	16	pobj	_	_
18	?	?	.	_	_	3	punct	_	_
"""
conlltree="""# What are the facts that you have to consider in your decision making process?
# 427, fr, 86011, 74
1	What	what	WP	_	_	2	attr	_	_
2	are	be	VBP	_	_	0	ROOT	_	_
3	the	the	DT	_	_	4	det	_	_
4	facts	fact	NNS	_	_	2	nsubj	_	_
5	that	that	WDT	_	_	9	dobj	_	_
6	you	-PRON-	PRP	_	_	7	nsubj	_	_
7	have	have	VBP	_	_	4	relcl	_	_
8	to	to	TO	_	_	9	aux	_	_
9	consider	consider	VB	_	_	7	xcomp	_	_
10	in	in	IN	_	_	9	prep	_	_
11	your	-PRON-	PRP$	_	_	14	poss	_	_
12	decision	decision	NN	_	_	13	compound	_	_
13	making	making	NN	_	_	14	compound	_	_
14	process	process	NN	_	_	10	pobj	_	_
15	?	?	.	_	_	2	punct	_	_
"""




#print(matchGrammar(sgno,conll.conll2tree(conlltree)))
#print(conll.conll2tree(conlltree).get(matchGrammar(sgno,conll.conll2tree(conlltree))[0]+1))
#qsdf

xmlToSentences("paula/FR_b2.xml")
xmlToSentences("paula/SP_b2.xml")
